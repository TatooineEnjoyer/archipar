...
export default function ContactForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const endpoint = process.env.NEXT_PUBLIC_LEADS_ENDPOINT?.trim();

    if (!endpoint) {
      setStatus("success");
      setStatusText(
        "Форма работает в демонстрационном режиме. Для реальной отправки подключите внешний endpoint."
      );
      return;
    }
    ...
    try {
      ...
      setStatus("error");
      setStatusText("Не удалось отправить заявку. Попробуйте ещё раз.");
    }
  }
